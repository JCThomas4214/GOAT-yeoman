...
export class <%= actionsname %>Actions {
	...
	*/
	...
	static EXAMPLE_STATE_SET: string = 'EXAMPLE_STATE_SET'; 
	...
	exampleSet(ex: string): void {
		this.ngRedux.dispatch(this.action(<%= actionsname %>Actions.EXAMPLE_STATE_SET, ex));
	}

	exampleErase(): void {
		this.ngRedux.dispatch(this.action(<%= actionsname %>Actions.EXAMPLE_STATE_ERASE));
	}
